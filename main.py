...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    # print the shape when the neural network is running, not before running
...
#    tf.Print(first_upsamplex2, [tf.shape(first_upsamplex2)])
    ...
    # replace pooling layer 4 to 1x1 Convolution to preserve spatial information
...
#    tf.Print(second_upsamplex2, [tf.shape(second_upsamplex2)])
    ...
    # replace pooling layer 3 to 1x1 Convolution to preserve spatial information
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    # create logits : reshape the 4D output tensor to 2D: each row represents a pixel and each column a class.
    logits = tf.reshape(nn_last_layer, (-1, num_classes))
    
    # make sure the target labels and prediction logits have same dimension
    ...
    # apply optimizer to loss function above
